...
if __name__ == '__main__':
    ...
    starttime = time.time()
    image = np.load(r'D:\Project\Python\Data\Data\sources\npy\1918.npy')
    image = image.astype(np.uint16)
    # 将灰度值归一化
    image = image / 65535
    # 双边滤波
    bfltImage = bfltGray(image, 3, 3, 20, 20)
    bfltImage  = bfltImage  * 65535
    bfltImage [bfltImage  > 65535] = 65535
    bfltImage  = np.round(bfltImage)
    endtime = time.time()
    print('time cost', endtime - starttime)
    bfltImage = bfltImage.astype(np.uint16)
    cv.imwrite(r'D:\Project\Python\Data\Data\sources\jbilateral\bilateral\bi1all332020.tif',bfltImage)
    np.save(r'D:\Project\Python\Data\Data\sources\jbilateral\bilateral\bi1all332020.npy',bfltImage)